        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('farmer.batches.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="shearing_date" name="shearing_date" type="date" class="mt-1 block w-full" :value="old('shearing_date')" required />
                            ...
                            <div>
                                ...
                                <select id="wool_type" name="wool_type" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                    ...
                                    <option value="merino" {{ old('wool_type') == 'merino' ? 'selected' : '' }}>Merino</option>
                                    <option value="crossbred" {{ old('wool_type') == 'crossbred' ? 'selected' : '' }}>Crossbred</option>
                                    <option value="lambswool" {{ old('wool_type') == 'lambswool' ? 'selected' : '' }}>Lambswool</option>
                                    <option value="other" {{ old('wool_type') == 'other' ? 'selected' : '' }}>Other</option>
                                </select>
                                <x-input-error class="mt-2" :messages="$errors->get('wool_type')" />
                            </div>

                            <div>
                                <x-input-label for="wool_weight" :value="__('Weight (kg)')" />
                                <x-text-input id="wool_weight" name="wool_weight" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('wool_weight')" required />
